...
test("demo username validates and returns to the selected lab", async ({
  page,
}) => {
...
test("malformed saved data recovers and return destinations cannot leave the site", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() =>
    localStorage.setItem("ciscoku:learner:v1", '{"version":99}'),
  );
